...
/**
 * Hide a listing while its card is checked out to a show. Tries quantity → 0
 * first (keeps the item id, watchers and history — needs the seller's
 * out-of-stock control enabled); if eBay rejects that, falls back to ENDING
 * the listing, logged so it can be relisted at check-in. Ownership-guarded.
 *
 * POST { itemId } → { ok, method: "quantity" | "ended" }
 */
export async function POST(request) {
  ...
  try {
    ...
    /* seller likely doesn't have out-of-stock control — fall through to end */
  }
