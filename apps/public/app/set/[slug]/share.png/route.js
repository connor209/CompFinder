...
import { setShareFields } from "@/lib/set-share";
import { loadSetCards, findSet } from "@/lib/card-page";

/**
 * What a link to a set page unfurls as in Discord, WhatsApp or a Facebook
 * group — the places a set link actually gets posted.
 *
...
async function archivo() {
  if (fontData === undefined) {
    try {
      ...
      console.error("set share.png: Archivo not on disk, falling back to the default face", err);
      fontData = null;
    }
  }
  return fontData;
...
export async function GET(request, { params }) {
  ...
  // A slug we don't publish has no page either, so nothing advertises this
  // image — but the URL is guessable and must not reach Supabase.
  if (!findSet(slug)) return new Response("Not found", { status: 404 });

  let set;
  try {
    set = await loadSetCards(slug);
  } catch {
    return new Response("Not found", { status: 404 });
  }
  if (!set || !set.cards.length) return new Response("Not found", { status: 404 });

  const fields = setShareFields({ set, cards: set.cards, now: new Date() });
  // Nothing priced yet is a real state and not a picture: rows would be empty
  // and the image would say only that we have heard of the set. The page falls
  ...
  return await settled(await image(fields), {
    // An hour at the edge. The numbers only move when the warmer runs, and a
    // set link doing the rounds in a group is one render rather than hundreds.
...
async function settled(response, headers) {
  try {
    ...
    console.error("set share.png: could not draw the set image", err);
    return new Response("Not found", { status: 404 });
  }
}
