 ...
 * the market. The dashed line is the median for the window on screen — the
 * same figure the recommendation is built from — which is what makes the
 * shape mean something.
 *
 ...
 */
export default function TrendChart({ sales, medianPence, windowDays = 90 }) {
  const days = useMemo(() => {
    const map = new Map();
  ...
  return (
    <svg className="trend" viewBox={`0 0 ${W} ${H}`} role="img" aria-label={`Sold price trend over the last ${windowDays} days`}>
      <defs>
      ...
      {days.map((d, i) => (
        <circle key={i} cx={x(d.t)} cy={y(d.v)} r={i === days.length - 1 ? 5 : 3.4}
          fill="var(--panel)" stroke="var(--accent)" strokeWidth={i === days.length - 1 ? 2.6 : 1.8} />
      ))}
