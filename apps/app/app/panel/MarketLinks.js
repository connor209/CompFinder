...
import { relFor } from "@compfinder/core/epn.js";
 ...
 */
export default function MarketLinks({ query, gameSlug = "pokemon", cardmarketId = null, sold = true, label = "", customId = "market-links" }) {
  const q = (query || "").trim();
  if (!q && !cardmarketId) return null;
  const ebayUrl = ebaySearchUrl(q, { sold, customId });
