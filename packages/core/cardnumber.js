/**
 * Collector-number guards, shared by both products.
 *
 * Moved here from apps/public/lib/tokens.js on 2026-08-25 unchanged, because
 * the business app needs them too and one app must never import another's lib.
 * Pure functions over a title and a number — no React, no Next, no Supabase —
 * which is what belongs in this package. tokens.js re-exports them, so every
 * existing caller on the public side is untouched.
 *
 * ESM, like most of this package. pricing.js and soldcomps.js are CommonJS;
 * see the note in CLAUDE.md about there being no "type" field on purpose.
 */
