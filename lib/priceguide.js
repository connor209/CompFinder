...
export const PREMIUM_LABELS = {
  holo: { short: "Rev. holo", long: "Cardmarket reverse-holo price" },
  foil: { short: "Foil", long: "Cardmarket foil price" }
};
export function premiumLabel(kind, key = "short") {
  if (!kind) return key === "short" ? "Premium" : "Cardmarket premium price";
  const known = PREMIUM_LABELS[kind];
  if (known) return known[key];
  const title = kind.charAt(0).toUpperCase() + kind.slice(1);
  return key === "short" ? title : `Cardmarket ${kind} price`;
}
