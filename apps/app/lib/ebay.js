...
/**
 * Revise the available quantity of a single active fixed-price listing
 * (Trading API ReviseInventoryStatus). Quantity 0 HIDES the listing from
 * buyers while keeping its item id, watchers and history — but only works
 * when the seller has eBay's out-of-stock control enabled; otherwise eBay
 * rejects it (callers fall back to ending the listing).
 */
export async function reviseItemQuantity(accessToken, itemId, quantity) {
  const safeItem = String(itemId).replace(/[^0-9]/g, "");
  if (!safeItem) throw new Error("Invalid item id.");
  ...
  const text = await res.text();
  if (!res.ok) throw new Error(`eBay quantity update failed (${res.status}).`);
  ...
  if (resp.Ack !== "Success" && resp.Ack !== "Warning") {
    const errs = resp.Errors;
    const msg = Array.isArray(errs) ? errs[0]?.LongMessage : errs?.LongMessage;
    throw new Error(msg || "eBay rejected the quantity update.");
  }
  return { ok: true, ack: resp.Ack };
}
