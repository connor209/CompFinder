 ...
 */
export const SKINS = [
  { key: "", name: "Terminal", light: "#F1F5F4", accent: "#0C7B6E" },
  { key: "holo", name: "Holo", light: "#F4F2FB", accent: "#7A4FE0" },
  { key: "tabletop", name: "Tabletop", light: "#EEF2E9", accent: "#2F7D4E" },
  { key: "vintage", name: "Vintage", light: "#F5EFE2", accent: "#B4622D" },
  { key: "glacier", name: "Glacier", light: "#EEF3F8", accent: "#1C6DD0" }
];
    ...
    <div className="skin-row" role="group" aria-label="Theme">
      {SKINS.map((s) => (
        <button key={s.key} className="skin-btn" aria-pressed={skin === s.key} onClick={() => choose(s.key)} title={s.name}>
          <span className="skin-sw" aria-hidden="true" style={{ background: `linear-gradient(135deg, ${s.light} 0 50%, ${s.accent} 50% 100%)` }} />
          {s.name}
        </button>
      ))}
    </div>
