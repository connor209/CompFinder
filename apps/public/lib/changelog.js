...
export const CHANGELOG = [
  {
    date: "2026-08-28",
    changes: [
      "Search for a graded card and you now get a graded price. Put a grade in your search — \"PSA 10 Umbreon VMAX 215/203\" — and until now we quietly threw away every sale of a slab, priced the raw card underneath it instead, and showed you that number as if it were the answer. On a card whose slabs sell for hundreds that could come back as a couple of pounds. Slab sales at the grade you asked for are now what the price is built from, raw copies are excluded, and the page says which. If there aren't enough sales at that grade we say so rather than falling back to the raw card — a wrong price is worse than none. Slabs listed the way the label reads — “PSA GEM MINT 10” rather than “PSA 10” — count too; the first version of this was missing them.",
      "There's a page listing every set now, at /sets — 92 of them, ordered by what their cards come to together, with the dearest card in each. Until now the only way to reach a set was to search a card first and click up from it, so if you didn't already know a card in the set, you couldn't get there at all. It's linked from the front page.",
      "Paste a link to a set — “most valuable cards in Prismatic Evolutions” — into a chat or a group and it now unfurls as a proper card: the set's five dearest cards with what they sell for, and the date. Card links have done that for a while; the set pages, which are the ones people tend to share, were still turning up as a bare address.",
      "The grade you type now survives the search itself. Putting “PSA 10” in front of a card's name confused the search — it could read “PSA” as the card's name and “10” as its number — and picking a card from the suggestions or the “which one?” screen quietly dropped the grade, answering with the raw card's price. Now the grade travels with you whichever way you reach the card, including from the Recent list — and the Save-image card says which grade its figure is for, so a screenshot of a slab's price can't be mistaken for the raw card's.",
